...
class PyJavascript:
    def __init__(self, filename='script.js'):
        self.script = ""
        self.filename = filename
        with open(filename, 'w+') as js:
            js.write(self.script)
            js.close()
    
    def update(self, filename="script.js"):
        with open(filename, 'w+') as js:
            js.write(self.script)
            js.close()
...
class Scripts:
    ...
    def change_variable(variable_name, action):
        return f"{variable_name} {action}"
    
    def func(func_name, parameters: list):
        __parameters = ""
        for parameter in parameters:
            __parameters += f", {parameter}"
        __parameters = __parameters[2:] if len(__parameters) >= 3 else ""
        return f"{func_name}({__parameters})"